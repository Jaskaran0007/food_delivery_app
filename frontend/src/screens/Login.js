...
export const Login = () => {
  // Using individual state hooks for email and password
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem("userEmail", json.email);  // Assuming the token is in the response as 'token'
        
        alert(json.message);  // Assuming response message is useful for the user

        // Redirect to home page or another protected route
        navigate('/');  // Change to your desired route
      } else {
        const json = await response.json();
        alert(json.message || "Login failed. Please check your credentials.");  // Display error message from API
        if (json.errors) {
